...
export default class SendMailUseCase {
  ...
  private async isDuplicate(key: string): Promise<{ isDuplicate: boolean; cachedResult?: any }> {
    try {
      ...
      if (SendMailUseCase.REDIS_REQUIRED) throw new Error('Redis unavailable and required for idempotency')
  ...
  private async cacheResult(key: string, result: any): Promise<void> {
    try {
      await this.cache.set(key, JSON.stringify(result), SendMailUseCase.CACHE_TTL)
    } catch (error) {
  ...
  async execute(data: SendMailRequest) {
    ...
    try {
      ...
      console.error('[SendMailUseCase] Error:', error)
